...
export const renderScores = async (game) => {
  ...
  if(value*(coins || 1) >= 100000){
    const winning = document.createElement('p')
    winning.textContent = `YOU WIN!`
    winning.style.marginBottom = '2rem'
    winning.style.marginTop = '-2rem'
    winning.style.fontSize = '1.6rem'

    wrapper.appendChild(winning)
  }
